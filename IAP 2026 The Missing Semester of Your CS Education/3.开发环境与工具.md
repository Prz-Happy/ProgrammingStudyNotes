---
created: 2026-07-31T08:50:12 (UTC +08:00)
source: https://missing-semester-cn.github.io/2026/development-environment/
---

# 开发环境与工具 · the missing semester of your cs education

# 开发环境与工具

*开发环境* 是一组用于开发软件的工具。开发环境的核心是文本编辑功能，以及配套的语法高亮、类型检查、代码格式化和自动补全等特性。像 [VS Code](https://code.visualstudio.com/) 这样的 *集成开发环境*（IDE）会把这些功能整合到一个应用中。基于终端的开发工作流则会把 [tmux](https://github.com/tmux/tmux)（终端复用器）、[Vim](https://www.vim.org/)（文本编辑器）、[Zsh](https://www.zsh.org/)（一种 Shell），以及语言特定的命令行工具组合在一起，例如 [Ruff](https://docs.astral.sh/ruff/)（Python 的 linter 和代码格式化工具）和 [Mypy](https://mypy-lang.org/)（Python 的类型检查器）。

IDE 和基于终端的工作流各有优劣。比如，图形化 IDE 往往更容易上手，而且如今的 IDE 一般也有更完善的开箱即用 AI 集成能力，例如 AI 自动补全；另一方面，基于终端的工作流更轻量，在没有 GUI 或无法安装软件的环境里，它们甚至可能是你唯一的选择。我们建议你对两者都建立基本熟悉度，并至少精通其中一种。如果你现在还没有偏好的 IDE，我们建议从 [VS Code](https://code.visualstudio.com/) 开始。

在这节课中，我们将介绍：

- [文本编辑与 Vim](https://missing-semester-cn.github.io/2026/development-environment/#text-editing-and-vim)
- [代码智能与语言服务器](https://missing-semester-cn.github.io/2026/development-environment/#code-intelligence-and-language-servers)
- [AI 驱动的开发](https://missing-semester-cn.github.io/2026/development-environment/#ai-powered-development)
- [扩展与 IDE 的其他功能](https://missing-semester-cn.github.io/2026/development-environment/#extensions-and-other-ide-functionality)

# 文本编辑与 Vim

编程时，你的大部分时间都花在代码间导航、阅读代码片段和修改代码上，而不是长时间连续输入，或者从头到尾线性地通读文件。[Vim](https://www.vim.org/) 正是针对这种任务分布而优化的文本编辑器。
...
**其他软件中的 Vim 支持。** 你不必真的使用 [Vim](https://www.vim.org/) 本体，也能从它的核心思想中受益。许多涉及文本编辑的软件都支持 “Vim mode”，要么是内建功能，要么是插件。例如，VS Code 有 [VSCodeVim](https://marketplace.visualstudio.com/items?itemName=vscodevim.vim) 插件，Zsh 内建了 [Vim 模拟支持](https://zsh.sourceforge.io/Guide/zshguide04.html)，就连 Claude Code 也内建了 [Vim 编辑模式](https://code.claude.com/docs/en/interactive-mode#vim-editor-mode)。你几乎可以认为，任何涉及文本编辑的工具，多少都会以某种方式支持 Vim mode。

## 模态编辑

Vim 是一个 *模态编辑器*：它针对不同类别的任务提供不同的工作模式。
...
Vim 的界面本身就是一种编程语言。按键操作（带有助记名）就是命令，而这些命令是可以 *组合* 的。这让移动和编辑都能非常高效，尤其是当这些命令变成肌肉记忆之后，就像你熟悉了键盘布局后，打字也会变得非常高效一样。
...
- 本课程上一轮迭代中的 [Vim 讲义](https://missing-semester-cn.github.io/2020/editors/)；我们在那里更深入地讲解了 Vim
...
- **代码质量。** 这些工具可以单独使用，但这类能力通常也由语言服务器提供。代码格式化会在你输入时自动缩进和整理格式，而类型检查器与 linter 会在你输入时发现代码中的错误。我们会在[代码质量](https://missing-semester-cn.github.io/2026/code-quality/)一讲中更深入地介绍这类功能。

## 配置语言服务器

对于某些语言，你只需要安装扩展和语言服务器，就万事俱备了。对另一些语言来说，若想把语言服务器的价值发挥到最大，你还需要告诉 IDE 你的环境信息。比如，让 VS Code 指向你的 [Python 环境](https://code.visualstudio.com/docs/python/environments)，就能让语言服务器看到你已安装的包。关于环境，我们会在[代码打包与发布](https://missing-semester-cn.github.io/2026/shipping-code/)一讲中更详细地介绍。

根据不同语言，你还可以为语言服务器配置一些设置。例如，在 VS Code 中使用 Python 支持时，你可以为那些没有使用 Python 可选类型注解的项目禁用静态类型检查。

# AI 驱动的开发

自 2021 年中 [GitHub Copilot](https://github.com/features/copilot/ai-code-editor) 引入 OpenAI 的 [Codex 模型](https://openai.com/index/openai-codex/) 以来，[大型语言模型（LLMs）](https://en.wikipedia.org/wiki/Large_language_model) 已被广泛应用于软件工程。目前主要有三种形态：自动补全、内联聊天和编程代理。
...
我们会在[Agentic Coding](https://missing-semester-cn.github.io/2026/agentic-coding/)一讲中深入介绍编程代理。

## 推荐软件

一些流行的 AI IDE 包括带有 [GitHub Copilot](https://github.com/features/copilot/ai-code-editor) 扩展的 [VS Code](https://code.visualstudio.com/)，以及 [Cursor](https://cursor.com/)。GitHub Copilot 目前对学生、教师以及热门开源项目的维护者提供[免费使用](https://github.com/education/students)。这是一个变化非常快的领域。许多头部产品的功能其实大致相当。

# 扩展与 IDE 的其他功能

IDE 本身已经是很强大的工具，而 *扩展* 会让它们更加强大。我们不可能在一节课里覆盖所有这些功能，但这里会先给出几个常见扩展的线索。我们鼓励你自己继续探索这一空间；网上有很多 IDE 热门扩展清单，例如针对 Vim 插件的 [Vim Awesome](https://vimawesome.com/)，以及[按安装量排序的 VS Code 扩展列表](https://marketplace.visualstudio.com/search?target=VSCode&category=All%20categories&sortBy=Installs)。

- [Development containers](https://containers.dev/)：主流 IDE 都支持它们（例如 [VS Code 就支持](https://code.visualstudio.com/docs/devcontainers/containers)）。dev containers 让你可以使用容器来运行开发工具，这对可移植性或隔离性都很有帮助。我们会在[代码打包与发布](https://missing-semester-cn.github.io/2026/shipping-code/)一讲中更深入地介绍容器。
...
2. 完成一道 [VimGolf](https://www.vimgolf.com/) 挑战。
3. 为你正在做的一个项目配置 IDE 扩展和语言服务器。确保所有预期功能都能正常工作，例如对库依赖进行跳转到定义。如果你手头没有可用于这个练习的代码，也可以从 GitHub 上找一个开源项目来用（例如[这个](https://github.com/spf13/cobra)）。
4. 浏览一份 IDE 扩展列表，安装一个看起来对你有用的扩展。

---

[Edit this page](https://github.com/missing-semester-cn/missing-semester-cn/blob/master/_2026/development-environment.md).
